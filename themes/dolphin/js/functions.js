...
function manageResults(x) {
	if(x == 0) {
		...
		var q = $("#search").val();
		document.location='index.php?a=search&q='+q.replace(' ','+');
	} else if(x == 2) {
		var q = $("#search").val();
		document.location='index.php?a=search&tag='+q.replace('#','');
	}
}
function liveSearch() {
	var q = $('#search').val();
...
function getNext(currentId, direction, uid) {
	...
	$('.gallery-footer-container').html('<div class="message-avatar">'+$('#avatar'+uid).html()+'</div><div class="message-top"><a onclick="gallery(\'close\')" title="Close Gallery"><div class="delete_btn"></div></a><a href="uploads/media/'+next.attr('id')+'" title="Download Image" target="_blank" download><div class="download_btn"></div></a><div class="message-author">'+$('#author'+uid).html()+'</div><div class="message-time">'+$('#time'+uid).html()+'</div></div>');
	jQuery('div.timeago').timeago();
